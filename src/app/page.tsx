...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const toggleScroll = (e: Event) => {
      const target = e.target as HTMLElement;
      // Không toggle nếu click vào các element tương tác (nút, link, input)
      if (target.closest('button') || target.closest('a') || target.closest('input') || target.closest('.modal')) {
        return;
      }
      if (isScrolling) {
        stopScroll();
    ...
    // Bắt đầu scroll sau 1 giây
    setTimeout(startAutoScroll, 1000);

    // Chạm/Click vào màn hình để toggle scroll
    window.addEventListener("click", toggleScroll);
    // Dừng scroll khi người dùng tự cuộn (lướt tay hoặc dùng chuột)
    window.addEventListener("wheel", stopScroll);
    window.addEventListener("touchmove", stopScroll);

    return () => {
      stopScroll();
      window.removeEventListener("click", toggleScroll);
